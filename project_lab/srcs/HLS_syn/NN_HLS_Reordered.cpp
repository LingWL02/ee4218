...
#pragma HLS INTERFACE axis port=M_AXIS
    ...
    ap_uint<8> hidden[NUM_SAMPLES][NUM_HIDDEN];          // 64 x 2
    ap_uint<8> output_data[NUM_SAMPLES][NUM_OUTPUTS];    // 64 x 1
...
#pragma HLS ARRAY_PARTITION variable=W_output dim=1 complete
    ...
    // ========== 1. READ HIDDEN LAYER WEIGHTS (8 x 2 = 16 words) ==========
...
#pragma HLS PIPELINE II=1
    ...
    // ========== 2. READ OUTPUT LAYER WEIGHTS (3 x 1 = 3 words) ==========
...
#pragma HLS PIPELINE II=1
    ...
    // ========== 3. READ INPUT DATA (64 x 7 = 448 words) ==========
...
#pragma HLS PIPELINE II=1
    ...
    hidden_layer: for (i = 0; i < NUM_SAMPLES; i++) {
        for (j = 0; j < NUM_HIDDEN; j++) {
#pragma HLS PIPELINE II=1
            acc = (ap_uint<16>)W_hidden[0][j] << 8;  // bias term

            hidden_dot_product: for (k = 0; k < NUM_INPUTS; k++) {
                acc += (ap_uint<16>)X[i][k] * (ap_uint<16>)W_hidden[k+1][j];
            }
...
#pragma HLS PIPELINE II=1
            acc = (ap_uint<16>)W_output[0][j] << 8;  // bias term

            output_dot_product: for (k = 0; k < NUM_HIDDEN; k++) {
                acc += (ap_uint<16>)hidden[i][k] * (ap_uint<16>)W_output[k+1][j];
            }
...
#pragma HLS PIPELINE II=1
            ...
            write_output.last = (i == NUM_SAMPLES - 1 && j == NUM_OUTPUTS - 1) ? 1 : 0;
