# PYNQ program to exercise AXI DMA with your matrix-vector HLS core
# Matches: input = A (row-major) then B, output = RES[ROWS]
# HLS behavior: acc += (A[i][j] * B[j]) / 256; RES[i] = acc & 0xFF
...
ROWS = 64
COLS = 8
NUMBER_OF_INPUT_WORDS  = ROWS * COLS + COLS
NUMBER_OF_OUTPUT_WORDS = ROWS

def main():
    # --- Load overlay and get DMA ---
    if len(sys.argv) == 2 and sys.argv[1] == "optim":
        print("Running HLS OPTIM")
        hls_name = "HLS OPTIM"
        overlay = Overlay('./lab4_hls_optim.xsa')
    elif len(sys.argv) == 1 :
        print("Running HLS UNOPTIM")
        hls_name = "HLS UNOPTIM"
        overlay = Overlay('./lab4_hls_unoptim.xsa')
    else:
        print("Invalid argument")
        return
    dma_hdl = overlay.axi_dma_0  # adjust if your DMA instance hls_name differs
    dma_hls = overlay.axi_dma_1  # adjust if your DMA instance hls_name differs
    ...
    # --- Test data (same as your C testbench) ---
    A = np.array([
    ...
    B = np.array([77, 210, 206, 233, 200, 15, 118, 136], dtype=np.int32)

    # --- Pack input buffer: A then B ---
    in_buf[:ROWS * COLS] = A.flatten()   # row-major by default
    in_buf[ROWS * COLS:] = B

    # --- Software reference (match HLS exactly) ---
    def expected_output(A, B):
        res = np.zeros((ROWS,), dtype=np.int32)
        for i in range(ROWS):
            acc = 0
            for j in range(COLS):
                acc += (int(A[i, j]) * int(B[j])) // 256
            res[i] = acc & 0xFF
        return res

    # --- Run DMA ---
    for name, dma in zip(["HDL", hls_name], [dma_hdl, dma_hls]):
        print(f"Testing DMA: {name}")
        start = time.time()
        ...
        print("Received:", out_buf)
        print("Expected:", exp)
        print("Match:", ok)
        print("Test Success" if ok else "Test Failed")
        print(f"DMA transfer time: {end - start:.6f} seconds\n")
