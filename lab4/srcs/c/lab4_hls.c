...
XAxiDma DmaInstance0;
XAxiDma DmaInstance1;
...
int main()
{
...
#ifndef SDT
    Status = InitDMA(&DmaInstance0, DMA_DEV_ID_0);
#else
    Status = InitDMA(&DmaInstance0, XPAR_XAXIDMA_0_BASEADDR);
#endif
    if (Status != XST_SUCCESS) {
        xil_printf("DMA 0 Initialization Failed\r\n");
        return XST_FAILURE;
    }

#ifndef SDT
    Status = InitDMA(&DmaInstance1, DMA_DEV_ID_1);
#else
    Status = InitDMA(&DmaInstance1, XPAR_XAXIDMA_1_BASEADDR);
#endif
    if (Status != XST_SUCCESS) {
        xil_printf("DMA 1 Initialization Failed\r\n");
        return XST_FAILURE;
    }
	...
	xil_printf("Dual DMA IP Implementation (HDL vs HLS)\r\n");
